...
const facebookloginreducer=(state={isLogIn:null,resp:null},action)=>{
    if(action.type==="FB_LOGIN"){
        
        return {...state,isLogIn:true,resp:action.payload}
    }
    if(action.type==="FB_LOG_OUT"){
      return {...state,isLogIn:false,resp:null}
    }
    return state
}
...
const mongologinreduxer=(state=intialStatelogin,action)=>{
  if(action.type==="SET_CURRENT_USER"){
    return {...state,isAuthenticated:!isEmpty(action.payload),user:action.payload,error:"",email:action.payload.email}
  }
  if(action.type===" USER_LOADING"){return {...state,loading:true}}
  if(action.type==="GET_ERRORS"){return {...state,error:action.payload}}
...
export default persistReducer(rootPersistConfig,combineReducers({
  ...
  //CreatePostReplayReducer:CreatePostReplayReducer
